...
def network_portrayal(G):
    ...
    def node_color(agent):
        return {State.INFECTED: "#FF0000", State.SUSCEPTIBLE: "#008000", State.DEAD: "#FFFF00"}.get(
            agent.state, "#808080"
        )
    ...
    def edge_color(agent1, agent2):
        if State.RESISTANT in (agent1.state, agent2.state):
            return "#000000"
        if State.DEAD in (agent1.state, agent2.state):
            return "#000000"
        return "#e8e8e8"
...
model_params = {
    ...
    "virus_letal_chance": mesa.visualization.Slider(
        "Virus Letal Chance",
        0.1,
        0.0,
        1.0,
        0.1,
        description="Virus Letal Chance",
    ),
